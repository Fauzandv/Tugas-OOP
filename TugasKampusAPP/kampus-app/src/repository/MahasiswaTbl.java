    ...
    public void update(String nim, Mahasiswa mahasiswa) {
        for(int i = 0; i < db.tables.dataMahasiswa.size(); i++) {
            if(db.tables.dataMahasiswa.get(i).getNim().equals(nim)) {
                db.tables.dataMahasiswa.set(i, mahasiswa);package repository;
...
public class MahasiswaTbl {
    ...
    public Mahasiswa getMahasiswaByidprodi(String idProdi) {
        for(Mahasiswa mahasiswa: db.tables.dataMahasiswa) {
            if(mahasiswa.getProdi().getIdprodi().equals(idProdi)){
                return mahasiswa;
            }
        }
        return null;
